...
import { AlertCircle, RefreshCw, ArrowLeft, BarChart3, Plane, Globe, Users, Package, Archive } from "lucide-react";
...
const stowagesYears = Array.from({ length: 2034 - 2016 + 1 }, (_, i) => 2016 + i);

const StowagesMarketDashboard = () => {
  const navigate = useNavigate();
  const [selectedYear, setSelectedYear] = useState(2025);
  const [activeTab, setActiveTab] = useState<MainTabType>("overview");
  const { data: marketData, isLoading, error, refetch } = useMarketData("/data/aircraft-stowages-market.json");
  ...
  const getSegmentInfo = () => {
    switch (activeTab) {
      ...
      case "application": return { data: marketData.application, title: "Stowage Type" };
  ...
  return (
    <div className="aircraft-interiors-theme min-h-screen">
      <ScrollToTop />
      <AircraftInteriorsDashboardHeader title="Aircraft Stowages Market" subtitle="Global Market Research Dashboard • 2016-2034" />
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <motion.footer initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="mt-12 border-t border-border pt-6">
          <div className="flex flex-col items-center justify-between gap-4 text-center md:flex-row md:text-left">
            <div>
              <p className="text-sm text-muted-foreground">Aircraft Stowages Market Research Report</p>
...
export default StowagesMarketDashboard;
