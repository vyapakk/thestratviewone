...
import { AlertCircle, RefreshCw, ArrowLeft, BarChart3, Plane, Globe, Users, Package, Bath } from "lucide-react";
...
const lavatoryYears = Array.from({ length: 2034 - 2016 + 1 }, (_, i) => 2016 + i);

const LavatoryMarketDashboard = () => {
  const navigate = useNavigate();
  const [selectedYear, setSelectedYear] = useState(2025);
  const [activeTab, setActiveTab] = useState<MainTabType>("overview");
  const { data: marketData, isLoading, error, refetch } = useMarketData("/data/aircraft-lavatory-market.json");
  ...
  const getSegmentInfo = () => {
    switch (activeTab) {
      ...
      case "application": return { data: marketData.application, title: "Lavatory Type" };
  ...
  const renderTabContent = () => {
    ...
    return (
      <SegmentDetailTab
        ...
        endUserLabel="End-User Type"
        equipmentLabel="Furnished Equipment Type"
        applicationLabel="Lavatory Type"
        useMillions
      />
    );
  };
  ...
  return (
    <div className="aircraft-interiors-theme min-h-screen">
      <ScrollToTop />
      <AircraftInteriorsDashboardHeader title="Aircraft Lavatory Market" subtitle="Global Market Research Dashboard • 2016-2034" />
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <motion.footer initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="mt-12 border-t border-border pt-6">
          <div className="flex flex-col items-center justify-between gap-4 text-center md:flex-row md:text-left">
            <div>
              <p className="text-sm text-muted-foreground">Aircraft Lavatory Market Research Report</p>
...
export default LavatoryMarketDashboard;
