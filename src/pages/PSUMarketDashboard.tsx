...
import { AlertCircle, RefreshCw, ArrowLeft, BarChart3, Plane, Globe, Users, Package, Wrench, Lightbulb } from "lucide-react";
...
const psuYears = Array.from({ length: 2034 - 2016 + 1 }, (_, i) => 2016 + i);

const PSUMarketDashboard = () => {
  const navigate = useNavigate();
  const [selectedYear, setSelectedYear] = useState(2025);
  const [activeTab, setActiveTab] = useState<MainTabType>("overview");
  const { data: marketData, isLoading, error, refetch } = useMarketData("/data/aircraft-psu-market.json");
  ...
  const getSegmentInfo = () => {
    switch (activeTab) {
      ...
      case "application": return { data: marketData.application, title: "Equipment Type" };
      case "equipment": return { data: marketData.furnishedEquipment, title: "Component Type" };
  ...
  const renderTabContent = () => {
    ...
    return (
      <SegmentDetailTab
        ...
        endUserLabel="End-User Type"
        equipmentLabel="Component Type"
        applicationLabel="Equipment Type"
        processTypeLabel=""
        useMillions
      />
    );
  ...
  return (
    <div className="aircraft-interiors-theme min-h-screen">
      <ScrollToTop />
      <AircraftInteriorsDashboardHeader title="Aircraft PSU Market" subtitle="Global Market Research Dashboard • 2016-2034" />
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <motion.footer initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="mt-12 border-t border-border pt-6">
          <div className="flex flex-col items-center justify-between gap-4 text-center md:flex-row md:text-left">
            <div>
              <p className="text-sm text-muted-foreground">Aircraft PSU Market Research Report</p>
...
export default PSUMarketDashboard;
