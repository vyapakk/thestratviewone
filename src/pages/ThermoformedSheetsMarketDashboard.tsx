...
const thermoformedSheetsTabs: TabConfig[] = [
...
const thermoformedSheetsYears = Array.from({ length: 2034 - 2018 + 1 }, (_, i) => 2018 + i);

const ThermoformedSheetsMarketDashboard = () => {
  const navigate = useNavigate();
  const [selectedYear, setSelectedYear] = useState(2025);
  const [activeTab, setActiveTab] = useState<MainTabType>("overview");
  const { data: marketData, isLoading, error, refetch } = useMarketData("/data/aircraft-thermoformed-sheets-market.json");
  ...
  return (
    <div className="aircraft-interiors-theme min-h-screen">
      <ScrollToTop />
      <AircraftInteriorsDashboardHeader title="Aircraft Interior Thermoformed Sheets Market" subtitle="Global Market Research Dashboard • 2018-2034" />
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <MainNavigation
            ...
            onYearChange={setSelectedYear}
            showYearSelector
            tabs={thermoformedSheetsTabs}
            years={thermoformedSheetsYears}
          />
        </div>
        ...
        <motion.footer initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="mt-12 border-t border-border pt-6">
          <div className="flex flex-col items-center justify-between gap-4 text-center md:flex-row md:text-left">
            <div>
              <p className="text-sm text-muted-foreground">Aircraft Interior Thermoformed Sheets Market Research Report</p>
...
export default ThermoformedSheetsMarketDashboard;
