...
const cabinLiningTabs: TabConfig[] = [
...
const cabinLiningYears = Array.from({ length: 2034 - 2016 + 1 }, (_, i) => 2016 + i);

const CabinLiningMarketDashboard = () => {
  const navigate = useNavigate();
  const [selectedYear, setSelectedYear] = useState(2025);
  const [activeTab, setActiveTab] = useState<MainTabType>("overview");
  const { data: marketData, isLoading, error, refetch } = useMarketData("/data/aircraft-cabin-lining-market.json");
  ...
  return (
    <div className="aircraft-interiors-theme min-h-screen">
      <ScrollToTop />
      <AircraftInteriorsDashboardHeader title="Aircraft Cabin Lining Market" subtitle="Global Market Research Dashboard • 2016-2034" />
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <MainNavigation
            ...
            onYearChange={setSelectedYear}
            showYearSelector
            tabs={cabinLiningTabs}
            years={cabinLiningYears}
          />
        </div>
        ...
        <motion.footer initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="mt-12 border-t border-border pt-6">
          <div className="flex flex-col items-center justify-between gap-4 text-center md:flex-row md:text-left">
            <div>
              <p className="text-sm text-muted-foreground">Aircraft Cabin Lining Market Research Report</p>
...
export default CabinLiningMarketDashboard;
